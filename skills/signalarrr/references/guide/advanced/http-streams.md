<!-- Generated from website/guide/advanced/http-streams.md by website/scripts/sync-skill.mjs. Do not edit; edit the docs page. -->
...
The clients authenticate these requests with their message credential — the one that travels with every hub call: `WithMessageCredential` in .NET, `messageCredential` in TypeScript, Swift and Kotlin, or the `credential` shortcut that sets it (see [the two credentials](../server/authorization.md#the-two-credentials)); the .NET Framework client uses the provider passed to `HARRRConnection.Create`. The connection credential is not used here. A credential without a space is sent as `Bearer <token>`; one that carries its own scheme (`Basic …`) is sent verbatim — the same rule the server applies to the credential in a message.
...
- [Item Streaming: Server-to-Client](../streaming/server-to-client.md) — IAsyncEnumerable patterns (different from file transfer)
- [Client Manager](../server/client-manager.md) — push files from controllers
- [Wire Protocol](../../reference/wire-protocol.md) — protocol details
