...
The clients authenticate these requests with their message credential — the one that travels with every hub call: `WithMessageCredential` in .NET and .NET Framework, `messageCredential` in TypeScript, Swift and Kotlin, or the `credential` shortcut that sets it (see [the two credentials](../server/authorization.md#the-two-credentials)). The connection credential is not used here. A credential without a space is sent as `Bearer <token>`; one that carries its own scheme (`Basic …`) is sent verbatim — the same rule the server applies to the credential in a message.
