<!-- Generated from website/guide/server/client-manager.md by website/scripts/sync-skill.mjs. Do not edit; edit the docs page. -->
...
clients: with a [backplane](./backplane.md) enabled, the filters and the send/invoke methods
...
> [what happens when a handler throws](../dotnet-client/server-to-client.md#what-happens-when-a-handler-throws).
...
| `AuthMode` | `AuthenticationMode` | How this connection authenticates — see [Authorization](./authorization.md) |
...
`RevalidationResult.Abort()`; see [Authorization](./authorization.md).
...
- [Server Methods](./server-methods.md) — server-to-client calls inside the hub
- [Authorization](./authorization.md) — filter clients by authentication state
- [Backplane & Clustering](./backplane.md) — enable multi-node routing and presence
- [Connection Setup (.NET)](../dotnet-client/connection-setup.md) — configure the .NET client
