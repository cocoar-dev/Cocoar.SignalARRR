<!-- Generated from website/guide/server/hub-setup.md by website/scripts/sync-skill.mjs. Do not edit; edit the docs page. -->
...
Every SignalARRR application needs at least one hub. The hub itself can be empty — actual method implementations go into [ServerMethods](./server-methods.md) classes.
...
> **Warning**
>
> Always call `base.OnConnectedAsync()` and `base.OnDisconnectedAsync()` — they manage the client registration in [ClientManager](./client-manager.md).
...
- [Server Methods](./server-methods.md) — split hub logic across classes
- [Authorization](./authorization.md) — protect methods with `[Authorize]`
- [Client Manager](./client-manager.md) — call clients from outside the hub
