using System.Collections.Generic;
using System.Linq;
using System.Text;
using Cocoar.SignalARRR.SourceGenerator.Model;

namespace Cocoar.SignalARRR.SourceGenerator.Emitters;

internal static class RegistrationEmitter {
    public static string EmitReferenced(IReadOnlyList<ContractInterfaceInfo> interfaces) {
        if (interfaces.Count == 0)
            return string.Empty;

        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine("// Proxies for [SignalARRRContract] interfaces discovered in referenced assemblies.");
        sb.AppendLine();
        sb.AppendLine("using System.Runtime.CompilerServices;");
        sb.AppendLine("using Cocoar.SignalARRR.ProxyGenerator;");
        sb.AppendLine();
        sb.AppendLine("namespace SignalARRR.Generated;");
        sb.AppendLine();
        sb.AppendLine("internal static class SignalARRRReferencedProxyRegistration");
        sb.AppendLine("{");
        sb.AppendLine("#pragma warning disable CA2255 // The 'ModuleInitializer' attribute should not be used in libraries");
        sb.AppendLine("    [ModuleInitializer]");
        sb.AppendLine("    internal static void Initialize()");
        sb.AppendLine("#pragma warning restore CA2255");
        sb.AppendLine("    {");

        foreach (var iface in interfaces) {
            sb.AppendLine($"        ProxyCreator.RegisterFactory<global::{iface.FullName}>(helper => new global::{iface.Namespace}.SignalARRR.{iface.ProxyClassName}(helper));");
        }

        sb.AppendLine("    }");
        sb.AppendLine("}");

        return sb.ToString();
    }

    public static string Emit(IReadOnlyList<ContractInterfaceInfo> interfaces) {
        if (interfaces.Count == 0)
            return string.Empty;

        // Use the first interface's namespace as the registration namespace
        var ns = interfaces[0].Namespace + ".SignalARRR";

        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine();
        sb.AppendLine("using System.Runtime.CompilerServices;");
        sb.AppendLine("using Cocoar.SignalARRR.ProxyGenerator;");
        sb.AppendLine();
        sb.AppendLine($"namespace {ns};");
        sb.AppendLine();
        sb.AppendLine("internal static class SignalARRRProxyRegistration");
        sb.AppendLine("{");
        sb.AppendLine("#pragma warning disable CA2255 // The 'ModuleInitializer' attribute should not be used in libraries");
        sb.AppendLine("    [ModuleInitializer]");
        sb.AppendLine("    internal static void Initialize()");
        sb.AppendLine("#pragma warning restore CA2255");
        sb.AppendLine("    {");

        foreach (var iface in interfaces) {
            sb.AppendLine($"        ProxyCreator.RegisterFactory<global::{iface.FullName}>(helper => new global::{iface.Namespace}.SignalARRR.{iface.ProxyClassName}(helper));");
        }

        sb.AppendLine("    }");
        sb.AppendLine("}");

        return sb.ToString();
    }
}
